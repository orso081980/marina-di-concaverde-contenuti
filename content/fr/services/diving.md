---
title: "Plongée — Marina di Conca Verde, Porto Pozzo"
description: "Cours et sorties de plongée en Gallura entre Santa Teresa Gallura et Palau. Fonds marins spectaculaires, grottes marines, posidonies et faune riche. Pour débutants et plongeurs expérimentés."
translations:
  it: "/servizi/diving"
  en: "/services/diving"
  de: "/dienstleistungen/tauchen"
  fr: "/services/diving"
hero:
  eyebrow: "Services · Plongée"
  title: "Plongée"
  subtitle: "Les plus beaux fonds marins de la Méditerranée, entre Santa Teresa Gallura et Palau, à quelques minutes de la résidence."
  image: "/img/diving.jpg"
  imageAlt: "Plongée fonds marins Gallura Sardaigne"
  primaryCta:
    label: "Réserver une sortie"
    href: "/contact"
  secondaryCta:
    label: "Tous les services"
    href: "/services"
textBlock:
  eyebrow: "Plongée"
  title: "Explorer les fonds marins de la Gallura"
stats:
  - value: "Certifié PADI"
    label: "Instructeurs qualifiés"
  - value: "Grottes Marines"
    label: "Sites spectaculaires"
  - value: "Posidonie"
    label: "Habitat protégé"
  - value: "Débutants à Pros"
    label: "Tous les niveaux"
---

Entre **Santa Teresa Gallura** et **Palau**, de nombreuses écoles et centres de plongée autorisés vous emmènent explorer des fonds marins extraordinaires.

## Les Fonds Marins de la Gallura

- **Prairies de posidonie** : indicateur d'eaux propres, abondantes le long de la côte de Porto Pozzo
- **Grottes et anfractuosités granitiques** : labyrinthes sous-marins habités par des mérous, murènes et pieuvres
- **Faune riche** : mérous, barracudas, pieuvres, oursins, homards, étoiles de mer
- **Épave de Spargi** : naufrage romain du Ier siècle av. J.-C.
